...
> “理论上，理论和实践是一样的。实际上，他们不是。

## 超越基础知识：应用环境工程

我们已经为上下文工程概念奠定了坚实的基础，从原子提示到认知工具。现在是时候看看这些原则如何应用于现实世界的挑战了，这些挑战突破了 LLM 的可能性界限。
...
创建长篇、连贯的内容突破了上下文管理的极限。让我们看看我们的原则是如何应用的：
...
此实现演示：
1. ** 使用基于 Schema 的方法**进行结构化问题解析
2. ** 使用显式中间状态**进行分步推理
3. **自我验证** 以检查每个阶段的工作
4. **内存管理** ，用于在整个求解过程中维护上下文
...
此实现演示：
1. ** 使用基于模式的方法进行**用户知识建模
2. ** 基于先决条件和用户状态**的自适应内容选择
3. ** 根据学习风格和知识量身定制的**个性化内容生成
4. ** 使用误解检测进行**响应评估

## 高级应用程序的关键模式

在这些不同的应用程序中，我们可以确定提高上下文工程有效性的常见模式：
...
1. **高级应用程序** 基于情境工程的基本原则构建
2. **状态管理** 对于复杂的应用程序变得越来越重要
3. **基于 Schema 的方法** 提供了处理复杂信息的结构
4. **多步骤处理将** 复杂的任务分解为可管理的部分
5. **自我验证** 提高了可靠性和准确性
6. **测量对于优化应用程序性能**仍然至关重要
...
在下一节中，我们将探讨提示编程，这是一种强大的方法，它将编程结构与提示的灵活性相结合，以创建更复杂的上下文工程解决方案。

[继续 07_prompt_programming.md →](07_prompt_programming.md)
