...
此模块实现了支持聊天机器人现场作的协议 shell。这些协议遵循结构化上下文作的 pareto-lang 格式，表示上下文工程的字段层。

## 协议 Shell 架构

协议 shell 用作作上下文字段的结构化作。每个协议都有一个特定的意图、定义的输入和输出，以及执行现场作的进程。
...
以下是我们的四个关键协议 shell 的实现：
1. `AttractorCoEmerge`： 识别并促进吸引子的共同出现
2. `FieldResonanceScaffold`：放大兼容模式之间的共鸣
3. `RecursiveMemoryAttractor`：通过吸引子实现内存持久化
4. `FieldSelfRepair`：检测并修复现场的不一致
...
我们实施的四个协议 shell 在一个协作生态系统中协同工作：
...
现在我们已经实现了协议 shell，我们需要创建 context field 实现来提供这些协议运行的 substrate。这将在模块中实现 `context_field.py` 。

协议 shell 和上下文字段之间的交互将展示字段作如何通过连续的语义作和紧急属性实现复杂的上下文工程。
