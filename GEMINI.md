...
本文档定义了 Gemini CLI 要使用的增强推理模式、协议 shell 和认知框架。这些工具提供结构化思维、分步推理和递归自我提升能力。
...
1. **框架选择**：根据手头的任务选择合适的认知框架。

2. **协议组合**：组合复杂任务的协议（例如， `research.technical` 后跟 `code.generate`）。

3. **递归改进**：应用 `self.reflect` 和其他递归协议以不断提高输出。

4. **上下文适应**：根据用户的专业知识和需求调整细节级别和重点。

5. **元认知**： `self.bootstrap` 在复杂任务开始时使用，以初始化最佳思维框架。

请记住，这些认知工具旨在具有可组合性和适应性。根据经验和反馈不断改进它们。
